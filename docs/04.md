## 4. APIGatewayとLambda関数の設定

APIGatewayから実行されるLambda関数を作成します。実際に動作を確認する目的で、まずは簡単はプログラムを作成してみます。

### 4.1.コードのダウンロード

ダウンロード先は別途ご案内します。

本セクションではコードの修正はありませんので、そのままLambdaに登録できます。コードのサンプルを下記に記載します。  
...
```  
### 4.2 AWS Lambdaの設定  
ToDo：設計図を用いたLambda関数の作成、ダウンロードしたzipファイルのアップロード
